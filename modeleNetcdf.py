...
class modeleNetcdf:
                                            ...
                                            if list_checked == len(column_name_list):
                                                ...
                                                if key == 'latitude':
                                                    ...
                                                    if all(self.dataframe[column].between(-90.0, 90.0)):
                                                        ...
                                                        if [column for column in self.dataframe.columns if key.split('_')[0] in column] not in column_name_list:
                                                            # Ajout de la liste
                                                            column_name_list.append([column for column in self.dataframe.columns if key.split('_')[0] in column])
                                                    # Sinon
                                                    else:
                                                        self.controleurlogs.add_log("Latitude values are not between -90 and 90.\n")
                                                        self.controleurlogs.add_colored_log("Latitude values are not between -90 and 90.\n", "red")
    ...
    def get_xarray_dataset(self: Self):
        ...
        self.xarray_dataset = modeleNetcdf.check_xarray_dataset(self.xarray_dataset)
    ...
    @staticmethod
    def create_xarray_dataset_dimension(arrangement: dict, xarray_dataset: xr.Dataset, dataframe: pd.DataFrame):
        ...
        for dimension_name in arrangement['dimension']:
            ...
            # Si la valeur de la dimension est une liste et si elle contient une ou plusieurs valeurs
                ...
                xarray_dataset.coords[dimension_name] = np.array(arrangement['dimension'][dimension_name]['values'])
        

    @staticmethod
    def create_xarray_dataset_variable(arrangement: dict, xarray_dataset: xr.Dataset, dataframe: pd.DataFrame):
        ...
        if specific_dimension_variable_dict:
            modeleNetcdf.insert_bidimensional_variable(specific_dimension_variable_dict, xarray_dataset, arrangement)


    @staticmethod
    def insert_bidimensional_variable(specific_dimension_variable_dict: dict, xarray_dataset: xr.Dataset, arrangement: dict):        
    ...
    @staticmethod
    def create_xarray_dataset_global_attribute(arrangement: dict, xarray_dataset: xr.Dataset):
    ...
    @staticmethod
    def create_xarray_dataset(dataframe: pd.DataFrame, arrangement_path: str):
        ...
        modeleNetcdf.create_xarray_dataset_dimension(arrangement, xarray_dataset, dataframe)

        modeleNetcdf.create_xarray_dataset_variable(arrangement, xarray_dataset, dataframe)
        
        modeleNetcdf.create_xarray_dataset_global_attribute(arrangement, xarray_dataset)
